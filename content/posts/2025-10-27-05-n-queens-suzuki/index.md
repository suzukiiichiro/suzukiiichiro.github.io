---
title: "Ｎクイーン問題（８９）Python/Codonで爆速プログラミング ビットでバックトラック"
date: 2025-10-27T13:25:09+09:00
draft: false
authors: suzuki
image: chess.jpg
categories:
...
* 列・対角衝突を**整数ビット**で管理する最小構成のビットボード版。
* 対称性（COUNT2/4/8）は未実装だが、**全解 Total は正確に算出可能**。
* Codon / PyPy 両対応。Codon 実行時は**型宣言で最適化**。
...
* 1 行あたり **O(1) 判定**で再帰を進められる。
* `fb/fc` 配列が不要になりメモリ**定数化**。
* **N=13 まで数十倍高速**（配列版比）。
