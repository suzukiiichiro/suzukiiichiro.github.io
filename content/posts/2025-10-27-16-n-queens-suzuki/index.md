---
title: "Ｎクイーン問題（１００）Python/Codonで爆速プログラミング コンステレーション＋マージ"
date: 2025-10-27T13:26:48+09:00
draft: false
authors: suzuki
image: chess.jpg
categories:
...
15Py をマージしました。
`SQB..` のバックトラック用関数を **`dfs` 関数に１本化**。
`exec_solutions` の呼び出し先を **`dfs`** に変更。
...
**結論**：codon for python 17Py_ は **GPU/CUDA `10Bit_CUDA/01CUDA_Bit_Symmetry.cu` と同等**の速度で動作します。
...
* エイト・クイーンのプログラムアーカイブ（Bash/Lua/C/Java/Python/CUDA など）
...
本プログラムは **開始コンステレーション（部分盤面）**で問題を分割し、各サブ問題を**ビットボード**で高速探索します。**Zobrist ハッシュ**や**正規化（Jasmin）**により重複抑止とキャッシュ効率を向上。

## 主要コンセプト / 実装の見どころ

* **ビットボード**
  左/右対角線・列をビットで管理。次行遷移では例として `dfs()` 内で
  `((ld|bit)<<1)`, `((rd|bit)>>1)`, `(col|bit)` を使用。

* **Zobrist ハッシュ**
  `_init_zobrist()` で N ごとのテーブル生成、`zobrist_hash()` で
  `ld/rd/col/LD/RD` を **N-bit に正規化後** XOR 加算。
...
* **コンステレーション列挙**
  ...
  `preset_queens` 個の事前配置を再帰展開。空きマスクは
...
* **対称性排除 & 正規化**
  ...
  `jasmin()`（`get_jasmin()` キャッシュ付）で代表形へ写像。

* **解探索分岐**
  `exec_solutions()` が j/k/l の位置や端からの距離に応じて
  旧 "SQ..." ラベル相当の分岐を選択し、**`dfs()` に委譲**。
  重複補正は `symmetry()`（90°: **2**, 180°: **4**, その他: **8**）で掛ける。

* **キャッシュ**
  サブ星座生成の再実行防止（`set_pre_queens_cached()` で `StateKey` セット）、
  さらに TXT/BIN キャッシュ（`load_or_build_constellations_*`）を用意。

## 設計上のポイント / レビュー

* **ハッシュの N-bit 正規化**
  `zobrist_hash()` 冒頭で `& ((1<<N)-1)` を徹底。負数や上位ビット汚染を回避。
...
* **Jasmin 正規化の指標**
  端からの距離 `ffmin(x, N-1-x)` を比較し回転回数 `arg` を決定 → 必要ならミラー。

* **分岐の抽象化**
  元の多数の再帰関数を **`dfs(funcname=...)`** で集約。
  核心は「**最下位 1bit 取り出し → 次行の遮蔽作成 → 先読み `_should_go_plus1()`**」。

* **先読みと追加遮蔽**
  `extra = _extra_block_for_row(...)` で j/k/l が来る行に追加ブロック。
  `_has_future_space_step()` により「次行が完全に詰んでいないか」を早期判定。

* **I/O の堅牢性**
  TXT/BIN の相互変換、BIN は **1 レコード = 16B** を `validate_bin_file()` で検査。
...
* 代表形写像：
  ...
  ```
* 追加遮蔽：
  `_extra_block_for_row(row_next, mark1, mark2, jmark, N)`
* 先読み：
  `_should_go_plus1(next_free, row_next, endmark, next_ld, next_rd, next_col, board_mask, extra)`
* 対称補正：
  `constellation["solutions"] = cnt * self.symmetry(ijkl, N)`

## 実行例（codon / AWS m4.16xlarge）
...
# 16Py_constellations_merge_codon.py（レビュー＆注釈）

## ✅ Opt-07：Zobrist/visited による探索済み検出

* 反映箇所：

  * `from typing import List, Set, Dict, Tuple`
    `StateKey = Tuple[int, int, int, int, int, int, int, int, int, int, int]`
  * `__init__`: `self.subconst_cache: Dict[StateKey, bool] = {}`
  * `set_pre_queens(...)` / `set_pre_queens_cached(...)`: `visited: Set[StateKey]`
  * キー：

    ```
    key = (ld, rd, col, row, queens, k, l, LD, RD, N, preset_queens)
    if key in visited: return
    visited.add(key)
    ```
* 現状は **タプル状態キー**での visited 判定。Codon でも安定・十分高速。
...
* データ構造：`visited`（Set：タプル or 圧縮キー）
  再帰木を跨いで同一状態の再訪を防止。

## ✅ Opt-10：Jasmin 正規化キャッシュ

* `get_jasmin(c, N)` / `jasmin_cache: Dict[Tuple[int,int], int]`
  回転・鏡映の正規化結果をメモ化。
...
* `load_constellations(...) / pickle`（Codon 事情により現在は停止）
* インスタンス内に辞書／集合の各種キャッシュを保持。

## ✅ Opt-13：部分盤面キャッシュ（tuple→dict）

* `set_pre_queens_cached(...)` で同一状態の再実行を抑止。

## ✅ Opt-14：星座の重複排除

* `if queens == preset_queens:` ブロック内で署名
  `signature = (ld, rd, col, k, l, row)` を照合して追加。

## ✅ Opt-15：Jasmin 正規化のメモ化

* `get_jasmin(c, N) → self.jasmin_cache[(c,N)]`

## ✅ Opt-16：visited の切替（zobrist/state_hash）

* A/B で `zobrist_hash` と `state_hash` を切替可能。
  `state_hash` は **O(1)** で高速・省メモリ、`zobrist_hash` は衝突耐性が高い。

## ✅ Opt-17：星座リストの外部キャッシュ（TXT/BIN）

* `save_constellations_txt/bin`, `load_constellations_txt/bin`
* `validate_constellation_list() / validate_bin_file()` で破損チェック。

## ✅ Opt-18：サブコンステレーションの tuple キーキャッシュ

* `self.subconst_cache: Dict[StateKey, bool] = {}`
* 全再帰で `set_pre_queens_cached(...)` を使用。

## ✅ Opt-19：星座の一意管理（signature）

* `self.constellation_signatures: Set[Tuple[int,int,int,int,int,int]]`
* 既存なら追加しない。

## ✅ Opt-20：Jasmin 変換キャッシュ（クラス属性）

* `self.jasmin_cache: Dict[Tuple[int,int], int]`
* `get_jasmin(c, N)` を経由して正規化。

## ✅ Opt-21：180°重複チェックの二重化削減

* 奇数 N の中央列ブロックで `rot180_in_set(...)` を削り、
  `check_rotations(...)` のみで十分。

## ✅ Opt-22：visited の粒度（星座ごとに new）

* 星座ごとに `visited: Set[StateKey] = set()` を新規化 → メモリ増大を回避。

## ✅ Opt-23：ビット演算のインライン化・共有

* `bit = x & -x`、`blocked → next_free` の短絡評価などを徹底。
* `symmetry(ijkl, N)` の小メモ化も有効。

## ✅ Opt-24：“先読み空き” の条件

* `if next_free and ((row >= endmark-1) or _has_future_space(...)):`
  ゴール直前は先読み不要。短絡評価でコスト最小化。

---

## 対応不要 / 設計上入れないもの

### ❎ 並列とキャッシュの整合（設計どおりで OK）

* `@par` は星座単位で独立。インスタンス属性キャッシュは
  生成段階（`gen_constellations`）で完結しており競合しない。

### ❎ 1行目以外の部分対称除去（未対応推奨）

* `jasmin/is_partial_canonical` による途中盤面の代表判定は、
  **bitmask-only** 設計と相性が悪く、過剰枝刈りのリスクが高い。
  現設計では導入しないのが安全。

### ❎ 軽量 `is_canonical()` の部分盤面チェック

* 行→列の履歴（`board[row] = col`）が無い構造では困難。
  現状の対称除去（初手左右半分・コーナー分岐・Jasmin）＋キャッシュで十分。

### ❎ マクロチェス（局所パターン）

* 直近行の列履歴が無い設計では一般化が難しい。
  既存の SQ 系分岐と対称除去でカバーできているため不要。
