---
title: "Ｎクイーン問題（８７）Python/Codonで爆速プログラミング バックトラック"
date: 2025-10-27T13:24:49+09:00
draft: false
authors: suzuki
image: chess.jpg
categories:
...
* **対角配列の長さは `2*size-1` を確保**し、境界外アクセスを回避。
...
* I/O は遅いため、大きな N は `printout()` を抑止した別モード推奨。
...
* `ld = row - col + self._off`, `rd = row + col` として境界を 0..2N-2 に統一しており安全。
...
* **ビットボード化:**
  `fa / fb / fc` を int に統合し、`free` から `bit = free & -free` で候補を高速列挙。
* **対称性削減と Unique 計算:**
  初手の左右半分化＋COUNT2/4/8。
