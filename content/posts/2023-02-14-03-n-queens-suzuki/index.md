...
## 考え方
これまでのクイーンの並びは、以下のような並びであってもカウントしていました（できたということにしていました）
...
さて４列にこれ以上「手がない」ということになり初めて、一つ手前３列のクイーンに軸が移動します。ここでは「↓」が右に一つずれます。ここでの動きはそれだけです。
```
       ↓
        column(列)
   _4___3___2___1___0_
...
(4,4)は(3,4)のクイーンのあたりなので移動できません。
<font color=red>「４列のクイーンはすべての可能性を探し終わった」</font>といえます。

ですので３列のクイーンはもう最下部へ到達しており、
...
先程と同様、手がない状態となると「一つ手前の」列である２列のクイーンが「ひとつずれてみようか」と下(2,3)に移動します。
...
```
ここからいつもの処理が始まります。
３列のクイーンは移動できる可能性を１から順番にたどります。
(3,0),(3,1),(3,2),(3,3),(3,4)
(3,0),(3,1)はあたりとなりますが、(3,2)が配置できますね。
```
           ↓
        column(列)
   _4___3___2___1___0_
...
```
  ...
  +-------------------+
```
さて、(3,2)のクイーンの位置で４列の移動できる可能性はすべて潰しました。
そこで、３列はひとつ下(3,3)にずれます。
が、あたりですね。さらにひとつ下の(3,4)にずれます。
```
           ↓
        column(列)
   _4___3___2___1___0_
...
```
  ...
  +-------------------+
```
４列目の移動できる可能性はすべて潰しました。
さらに３列目(3,4)のクイーンは最下部となりました。
もう動ける可能性のある場所はありません。

<font color=red>「２列(2,3)のクイーンはこれ以上手がない」状態です。</font>

ということで２列(2,3)のクイーンは(2,4)へ移動します。
盤面は以下のとおりです。
...
ではいつものとおり３列目の一番上から可能性をたどっていきます。
(3,0),(3,1),(3,2),(3,3),(3,4)
(3,2)に置けそうです。
```
           ↓
...
```
  ...
  +-------------------+
```
ではあらためての繰り返しですが、４列目のクイーンの移動の可能性を上から順番にたどります。
(4,0),(4,1),(4,2),(4,3),(4,4)
(4,2)が置けそうです。
これで一つ解が発見できました。
...
さらに、２列目のクイーンの移動できる可能性はすべて潰しました。
ここで軸「↓」が一つ左にずれて１列が軸となり、(1,1)のクイーンをひとつ下に移動させます。
盤面は以下のとおりです。
...
## Ｎが５のときの計算方法
前回に加えて、横の効きも取り入れたことで計算方法が少し変わります。
縦に一つのクイーン、いわゆる縦の効きだけを考慮した場合のＮ５の計算方法は

５＊５＊５＊５＊５＝３１２５
でした。いわゆるＮ＾５＝３１２５　となります。
