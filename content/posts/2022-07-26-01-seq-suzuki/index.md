...
|オプション    |意味|
|--------------|----|
|-w|先頭を0で埋めて、数字の幅を等しくする（本文参照）|
|-f 書式|数字の書式を指定する（本文参照）|
|-s 文字列|数字の区切りに使う文字列を指定する（デフォルトは改行|

## seqコマンド詳細説明

### 連続番号を出力する 
　「seq 10」で1から10までを出力します。
...
　書式には「%e」（指数形式、大文字で出力するときは「%E」）、「%f」（小数点形式）、「%g」（一般的な数値の形式）が使用できます。これは浮動小数点用の書式です。
