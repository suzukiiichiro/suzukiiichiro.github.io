---
title: "【split】bash/シェルスクリプトマニアックコマンドあれこれ４"
date: 2022-11-08T10:57:58+09:00
draft: true
authors: suzuki
image: bash.jpg
categories:
...
## splitコマンド
`split` コマンドは、ファイルを指定したサイズや個数に分割します。
ファイルが大きすぎてコピーやダウンロードに失敗するような時や、小容量のメディアに保存する場合、添付ファイルの容量に制限があるメールサービスなどで使用します。
分割するファイルの種類には特に制限はありません。
テキストファイルを分割することもできます。
