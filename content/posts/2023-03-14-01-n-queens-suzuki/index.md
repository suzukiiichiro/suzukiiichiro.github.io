...
多くのエイトクイーン研究者は盤面の縦横幅を８に固定せず、 ＮxＮの盤面とし、現実的な処理時間で、どのサイズ（Ｎ）までの解を求めることができるかを競っている。
現在、Ｎ２８の解を求めたドレスデン大学が世界一を記録している。詳しくは（１）を参照
...
## ブルートフォースについて
力まかせ探索は、全ての可能性のある解の候補を体系的に数えあげる。
解の候補を１つ数え上げる都度、その解の候補が解となるかをチェックする方法。
解候補の生成後に解であり得るかをチェックするので「生成検査法（generate and test）」とも呼ばれる。

ブルートフォースの手数は、各行には１個のクイーンしか置けない（同じ行に２つのクイーンを置くと、その時点でアウト）ので、Ｎ５の場合は、５＊５＊５＊５＊５＝３１２５となり、５＾５，またはＮ＾Ｎと表記する。
したがって、アルゴリズム的に言えば、「力まかせ探索による Ｎクイーン問題の処理時間は O(N^N) 」となる。


## 効き筋のチェック
ここでは具体的な説明を割愛するが、解の候補が導き出される都度、以下の効き筋チェック関数が呼び出され、解となりうるかを判定する。いわゆる３１２５ステップのＮ５の場合、以下のチェック関数は３１２５回実行される。
...
各行について、他の行と比べて、同じ `col` に置かれてないか、斜め45度方向に置かれていないかをチェックし、効きがあればその時点で 「０」（false)を返します。
...
プログラムソースは比較的シンプルに書くことができるのですが、「自分で自分を呼び出す」というロジックが馴染めない人も多いのではないかと思います。
さらに、何が行われているのかはわかりにくく、「ここは再帰」ということで理解をしてきたことも否めません。
では、再帰で書くしかないのでしょうか。
...
なにより、ＣＰＵでの計算よりもおよそ１００倍以上高速なＧＰＵでの計算ができます。
ＧＰＵで処理するためにはＧＰＵ−ＣＵＤＡというミドルウェアを使う必要がありますが、ＧＰＵ−ＣＵＤＡは「再帰」に未対応なのです（昔は対応していました）

ということで、第二章からは「再帰」と「非再帰」を両面で理解しつつ、進めていきたいと思います。
...
`check_bluteForce()` 関数は、クイーンの効きを判定する関数となります。
...
`check_bluteForce()` の末尾では以下のようになっています。
...
`check_bluteForce()`関数で、効きの判定を行い、「戻り値が `$?==1` で、効きではないのと判定されたので解を発見したことになる、よって`((TOTAL++))`で合計をインクリメントして、`printRecord()`でボードレイアウトによってクイーンの位置を表示するというロジックとなります。
...
`bash` では、`return` よって値を戻りねとして扱うことができないと言われていますが、こうした使い方によって、関数の戻り値を使うことができることを覚えておくと良いでしょう。
...
３１２５回クイーンの配置が完了する都度、`check_bluteForce()`が呼び出されるわけです。
...
次のページでは、同様に危機の判定関数を別関数にして、バックトラックの再起と非再帰をご説明します。
