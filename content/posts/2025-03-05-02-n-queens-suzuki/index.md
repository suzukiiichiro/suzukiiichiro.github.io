---
title: "Ｎクイーン問題（６７）Python-codonで高速化 01Python_bluteForce"
date: 2025-03-05T12:31:19+09:00
draft: false
authors: suzuki
image: chess.jpg
categories:
...
## ブルートフォース（ちからまかせ探索）をpython/pypy/codonで速度比較
...
## pythonで実行
```
$time python 01Python_bluteForce.py
:
:
16777210: 77777771
16777211: 77777772
...
pypyで実行
```
$time pypy 01Python_bluteForce.py
:
:
16777210: 77777771
16777211: 77777772
...
```
$time codon build -release 01Python_bluteForce.py && ./01Python_bluteForce
:
:
16777211: 77777772
16777212: 77777773
...
## 実行結果
```
python real    4m48.897s
pypy   real    5m22.720s
codon  real    1m43.534s
```

pypyの良さを引き出すことができなかったようです。とはいえcodonの速度は十分に引き出すことができました。

ではcodonによる高速化の手順を説明していきます。
...
2.型付けを明示的に行う
これはcodon化の作業の９０％となります。
pythonは型付けを行わなくてもpythonが判定して動作します。それが遅さの原因です。明示的に片付けを行いましょう。例えば`nqueens()`にわたすパラメータは以下のように書きます。

``` python
class NQueens01:
  :
  :
  # def nqueens(self,row):    # pythonの書き方
  def nqueens(self,row:int):  # codonの書き方
    :
    :
if __name__ == '__main__':
  NQueens01().nqueens(0)
...
3. グローバル変数をなくそう
完全になくせないまでも無くす努力をすべきです。
この章ではグローバル変数をあえて使いますが、あればあるほど遅くなります。ローカル変数を使って、関数に変数をパラメータ渡しする方法を懸命に考えましょう。このことに関してはいずれしっかりと説明していきます。

4. グローバル変数は`__init__(self):`関数で初期化します
同時にグローバル部分で型付けを行います。

``` python
class NQueens01:
  size:int          # int型
  aboard:list[int]  # list[int]型
  count:int         # int型
  def __init__(self):
    self.size=8
    self.aboard=[0 for i in range(self.size)]
