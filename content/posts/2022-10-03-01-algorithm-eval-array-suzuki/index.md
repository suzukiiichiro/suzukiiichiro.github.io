...
とすると、名前をセットしたりすることもできますが、Bashではできなかったりします。
アルゴリズムを進めていくと、一つの配列に、IDと値をセットしたり、参照したり、置き換えたりしたくなります。
この章では、そうした「ふるまい」をBashで実現してみようということになります。
...
同じですね。
今回の２次元配列版では、前章の配列版よりもきちんと記述されています
